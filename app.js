...
const lessons=sessions.flatMap(session=>[
  ...(session.videoUrl?[{id:`${session.number}-video`,round:`第${session.number}回`,title:session.videoTitle,type:'video',url:session.videoUrl,done:false}]:[]),
...
function renderAnswers(){const root=document.querySelector('#answers-root');if(!root)return;const works=lessons.filter(lesson=>lesson.type==='work');root.innerHTML=works.map(work=>{const saved=JSON.parse(localStorage.getItem(`work-${work.id}`)||'{}');const hasAnswers=Object.values(saved).some(value=>String(value).trim());const answers=work.questions.map((question,index)=>{if(question.fields){return question.fields.map((field,fieldIndex)=>({question:`質問${index+1}：${question.question}`,label:field,value:saved[`${index}-${fieldIndex}`]}))}return [{question:`質問${index+1}：${question.question}`,label:question.label||'選んだ回答',value:saved[index]}]}).flat();return `<section class="answer-card"><div class="answer-head"><div><span>${work.round}</span><h2>${work.title}</h2></div><span class="answer-status ${hasAnswers?'saved':'empty'}">${hasAnswers?'保存済み':'未回答'}</span></div>${hasAnswers?`<div class="answer-list">${answers.filter(answer=>String(answer.value||'').trim()).map(answer=>`<div class="answer-item"><small>${answer.question}</small><b>${answer.label}</b><p>${escapeHtml(answer.value).replace(/\n/g,'<br>')}</p></div>`).join('')}</div>`:`<p class="answer-empty">このワークには、まだ保存された回答がありません。</p>`}<a class="answer-edit" href="lectures.html?work=${work.id}">${hasAnswers?'回答を確認・編集する':'ワークに回答する'} →</a></section>`}).join('')}
...
document.addEventListener('DOMContentLoaded',()=>{guardAdmin();renderLessons();renderAdmin();renderAnswers();const workId=new URLSearchParams(location.search).get('work');if(workId)setTimeout(()=>openLesson(workId),0)});
