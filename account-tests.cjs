...
assert.equal(run('ayanoSessions.every(s=>s.previouslyCompleted && !s.videoUrl && s.workQuestions.every(q=>!q.examples))'),true);
...
listeners.DOMContentLoaded();
assert.equal(run('lessons.length'),30);
assert.equal(run('lessons.every(l=>l.type==="work" && lessonDone(l))'),true);
...
assert.equal(elements['#home-count'].textContent,'30 / 30 完了');
